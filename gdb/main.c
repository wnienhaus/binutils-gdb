	  ...
	  case OPT_XTENSACONFIG:
	    {
              ...
              xtensa_register_t *regmap = (xtensa_register_t *) xtensa_load_config ("rmap", NULL);
              if (regmap)
              {
                xtensa_tdep = xtensa_gdbarch_tdep (regmap);
              }
              else
              {
                error (_("Can not load xtensa registers map! Check xtensa-module library."));
              }
	    }
	    break;
#endif
